...
CREATE TABLE quotes (
  ...
  contractor_notes TEXT,                -- internal notes visible only to contractor staff
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
  updated_at TEXT NOT NULL DEFAULT (datetime('now'))
);
...
CREATE TABLE quote_activity (
  ...
  actor_email TEXT,                   -- for super admin actions not in staff table
...
-- Single source of truth for super admin access.
...
-- Audit events (compliance logging for staff/contractor/super admin actions)
...
CREATE INDEX idx_quotes_active ON quotes(contractor_id, deleted_at);
...
CREATE INDEX idx_audit_events_actor ON audit_events(actor_email);
